...
| 28 | Antigravity | Add status filter to Explorer table | Added a threshold status dropdown filter to the Timeseries Bucket Rows table header | Verified rows filter correctly and count displays update |
| 29 | Antigravity | Fix empty active sources fallback | Prevented dashboard from querying the backend and falling back to all data when no devices are active | Verified dashboard displays empty state correctly when all devices are disabled |